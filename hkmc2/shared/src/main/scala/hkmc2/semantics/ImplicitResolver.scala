        ...
        case S(sign) => resolveType(sign) match
          case S(tpe: Type.Concrete) =>
            ictx.get(tpe) match
              ...
              case N =>
                raise(ErrorReport(
                  msg"Missing instance for contextual parameter ${p.showDbg}" -> p.toLoc ::
                  msg"Required by module method application at: " -> t.toLoc ::
                  msg"Expected: ${tpe.toString()}; Available: ${ictx.iEnv.toString()}" -> N :: Nil))
          case N =>
        case N =>
  ...
  def resolveType(t: Term): Opt[ICtx.Type.Concrete] = t match
      ...
      case Term.TyApp(con, args) => 
        (resolveType(con), args.map(resolveType(_)).sequence) match
          ...
          case _ =>
            // Either the type constructor or the arguments are not resolved.
          ...
          case S(tpe: TypeAliasSymbol) =>
            tpe.defn match
              ...
              case N => 
                ...
                N
          case N =>
            raise(ErrorReport(msg"Expected a type, got ${t.showDbg}" -> t.toLoc :: Nil))
            N
        case N =>
          raise(ErrorReport(msg"Expected a type symbol, got ${t.showDbg}" -> t.toLoc :: Nil))
          N
